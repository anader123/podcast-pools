...
// .env Variables
const { WEB3_PROVIDER, MINT_PRIV_KEY } = process.env;
...
// Contract Instances
const intoEtherTokenContract = new ethers.Contract(
...
const mintToken = async (ipfsHash) => {
    try {
        const resultTx = await intoEtherTokenContract.mintEpisode(
            PRIZE_POOL_ADDRESS,
            ipfsHash
        );
        await resultTx.wait();
...
const addERC721ToPrizePool = async () => {
    try {
        ...
        const addTx = await strategyContract.addExternalErc721Award(
            INTO_ETH_TOKEN_ADDRESS,
            tokenIdAr
        );
        await addTx.wait();
...
const startAndAwardPrize = async () => {
    try {
        ...
        // Delay second tx to give the first one time to settle
        setTimeout(async () => {
