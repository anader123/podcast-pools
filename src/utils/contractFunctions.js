...
export const initializeContracts = () => {
    try {
        ...
        strategyContract = new ethers.Contract(PRIZE_STRAT_ADDRESS, PodPrizeStratAbi.abi, signer);
        poolContract = new ethers.Contract(PRIZE_POOL_ADDRESS, PodPrizePoolAbi.abi, signer);
        IntoEthToken = new ethers.Contract(INTO_ETH_TOKEN_ADDRESS, IntoEthTokenAbi.abi, signer);
        DaiToken = new ethers.Contract(DAI_TOKEN_ADDRESS, ERC20Abi.abi, signer);
        TicketToken = new ethers.Contract(TICKET_TOKEN_ADDRESS, ERC20Abi.abi, signer);
    } catch (error) {
...
export const getTotalTicketCount = async () => {
    try {
        const result = TicketToken.totalSupply();
...
export const getCurrentNftPrizes = async () => {
    const ipfsHashArray = [];
    try {
        const idArray = await strategyContract.getAwardErc721TokenIds(INTO_ETH_TOKEN_ADDRESS);
